...
volatile UINT messageHookReferenceCount = 0;
...
HWND GetControlParentWindow(LPOLEOBJECT pControl)
{
	HWND hWndParent = NULL;

	CComPtr<IOleClientSite> pClientSite;
	if(SUCCEEDED(pControl->GetClientSite(&pClientSite))) {
		CComQIPtr<IOleControlSite> pControlSite = pClientSite;
		if(pControlSite) {
			CComPtr<IDispatch> pExtendedControl;
			if(SUCCEEDED(pControlSite->GetExtendedControl(&pExtendedControl))) {
				DISPID dispid;
				LPOLESTR propertyName = OLESTR("Parent");
				if(SUCCEEDED(pExtendedControl->GetIDsOfNames(IID_NULL, &propertyName, 1, LOCALE_SYSTEM_DEFAULT, &dispid))) {
					DISPPARAMS dispParams = {0};
					VARIANT result;
					VariantInit(&result);
					if(SUCCEEDED(pExtendedControl->Invoke(dispid, IID_NULL, LOCALE_SYSTEM_DEFAULT, DISPATCH_PROPERTYGET, &dispParams, &result, NULL, NULL))) {
						CComQIPtr<IOleWindow> pParent = result.pdispVal;
						if(pParent) {
							pParent->GetWindow(&hWndParent);
						}
					}
					VariantClear(&result);
				}
...
BOOL IsValidBandIndex(int bandIndex, ReBar* pReBar)
{
	...
	if(pReBar) {
		OLE_HANDLE h = NULL;
		pReBar->get_hWnd(&h);
		hWndRBar = static_cast<HWND>(LongToHandle(h));
	}

	return IsValidBandIndex(bandIndex, hWndRBar);
}

BOOL IsValidBandIndex(int bandIndex, HWND hWndRBar)
{
	if(bandIndex < 0 || !IsWindow(hWndRBar)) {
		return FALSE;
	}

	return (bandIndex < static_cast<int>(SendMessage(hWndRBar, RB_GETBANDCOUNT, 0, 0)));
}
...
BOOL IsValidButtonIndex(int buttonIndex, ToolBar* pToolBar)
{
	...
	if(pToolBar) {
		OLE_HANDLE h = NULL;
		pToolBar->get_hWnd(&h);
		hWndTBar = static_cast<HWND>(LongToHandle(h));
	}

	return IsValidButtonIndex(buttonIndex, hWndTBar);
}

BOOL IsValidButtonIndex(int buttonIndex, HWND hWndTBar)
{
	if(buttonIndex < 0 || !IsWindow(hWndTBar)) {
		return FALSE;
	}

	return (buttonIndex < static_cast<int>(SendMessage(hWndTBar, TB_BUTTONCOUNT, 0, 0)));
}
...
UINT GetBarMessage(void)
{
	...
	if(message == 0) {
		message = RegisterWindowMessage(_T("Bar_InternalGetInstanceMessage"));
	}
	return message;
}
...
UINT GetControlHostWindowMessage(void)
{
	...
	if(message == 0) {
		message = RegisterWindowMessage(_T("ControlHostWindow_InternalGetInstanceMessage"));
	}
	return message;
}
...
UINT GetAutoPopupMessage(void)
{
	...
	if(message == 0) {
		message = RegisterWindowMessage(_T("ToolBar_InternalAutoPopupMessage"));
	}
	return message;
}
...
UINT GetMDIChildWindowStateChangedMessage(void)
{
	...
	if(message == 0) {
		message = RegisterWindowMessage(_T("ReBar_InternalMDIChildWindowStateChangedMessage"));
	}
	return message;
}
...
UINT GetDisplayChevronPopupMessage(void)
{
	...
	if(message == 0) {
		message = RegisterWindowMessage(_T("ToolBar_DisplayChevronPopupMessage"));
	}
	return message;
}
...
UINT GetDeferredSetButtonTextMessage(void)
{
	...
	if(message == 0) {
		message = RegisterWindowMessage(_T("ToolBar_DeferredSetButtonTextMessage"));
	}
	return message;
}
...
{
	InterlockedIncrement(&messageHookReferenceCount);
	if(!hMessageHook) {
		hMessageHook = SetWindowsHookEx(WH_GETMESSAGE, GetMsgProc, ModuleHelper::GetModuleInstance(), GetCurrentThreadId());
		ATLASSERT(hMessageHook);
	}
}

void RemoveMessageHookClient(void)
{
	ATLASSERT(messageHookReferenceCount > 0);
	if(InterlockedDecrement(&messageHookReferenceCount) == 0) {
		if(hMessageHook) {
...
LRESULT CALLBACK GetMsgProc(int code, WPARAM wParam, LPARAM lParam)
{
	...
	if(code == HC_ACTION && wParam == PM_REMOVE && pMessage->message != GetBarMessage() && pMessage->message != WM_FORWARDMSG) {
		// find the rebar or tool bar control that is the target window or the parent of the target window
		...
		while(!hWndControl && hWnd) {
			controlType = 0;
			hWndControl = reinterpret_cast<HWND>(::SendMessage(hWnd, GetBarMessage(), reinterpret_cast<WPARAM>(&controlType), reinterpret_cast<LPARAM>(&wantsForwardMsg)));
			hWnd = ::GetParent(hWnd);
		}

		if(wantsForwardMsg) {
			if(hWndControl && ::IsWindow(hWndControl)) {
				SendMessage(hWndControl, WM_FORWARDMSG, 0, lParam);
			}
		}
	}

	if(!consumeMessage) {
		lr = CallNextHookEx(hMessageHook, code, wParam, lParam);
	}
	return lr;
}