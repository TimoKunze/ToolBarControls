...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		...
		if(properties.pOwnerToolBar) {
			properties.pOwnerToolBar->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		}
	...
	if(properties.numberOfItemsToDisplay > 1) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = *pFormat;
			...
			if(properties.pOwnerToolBar) {
				properties.pOwnerToolBar->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			}
...
SourceOLEDataObject::Properties::~Properties()
{
	...
	#endif
	if(pOwnerToolBar) {
		pOwnerToolBar->Release();
	}
}

void SourceOLEDataObject::SetOwner(ToolBar* pOwner)
{
	if(properties.pOwnerToolBar) {
		properties.pOwnerToolBar->Release();
	}
	properties.pOwnerToolBar = pOwner;
	if(properties.pOwnerToolBar) {
		properties.pOwnerToolBar->AddRef();
	}
}
	...
	switch(format.cfFormat) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = format;
			...
			if(properties.pOwnerToolBar) {
				properties.pOwnerToolBar->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			}
